package org.lushen.mrh.example.netty.http.server.netty.filter;

import java.net.InetSocketAddress;

import org.apache.commons.lang3.ArrayUtils;
import org.lushen.mrh.example.netty.http.server.netty.HttpFilter;
import org.lushen.mrh.example.netty.http.server.netty.HttpFilterChain;
...
public class HttpWhiteListIpsFilter implements HttpFilter {
	...
	public void doFilter(ChannelHandlerContext ctx, HttpRequest request, HttpFilterChain chain) throws Exception {
		...
		if( ! ArrayUtils.contains(this.whiteListIps, ipAddress)) {
			throw new Exception(String.format("Invalid IP Address [%s] !", ipAddress));
		}
